...
def test_upload_file_zh_error_scenarios():
    ...
    # 备份原始API密钥
    original_api_key = os.getenv("NUOANAI_KEY")

    try:
        # 设置无效的API密钥
        os.environ["NUOANAI_KEY"] = "invalid_api_key_123"
        ...
        # 恢复原始API密钥
        if original_api_key:
            os.environ["NUOANAI_KEY"] = original_api_key
        else:
            if "NUOANAI_KEY" in os.environ:
                del os.environ["NUOANAI_KEY"]
...
def test_different_file_types():
    ...
    if not api_key:
        print("❌ 错误: 未找到NUOANAI_KEY环境变量，跳过此测试")
        return False
